...
if  [[ "$build_diff_kernel" == true ]]; then
  ...
  cd $REPOROOT/src/python_routing_v02/fast_reach/fortran2cython_test3_array/fortran_code/
  make clean
  make diffusive.o
  make pydiffusive.o
  make install || exit
...
if [[ "$build_routing" == true ]]; then
  ...
  cd $REPOROOT/src/python_routing_v02/fast_reach/fortran2cython_test3_array/
  rm -rf build
