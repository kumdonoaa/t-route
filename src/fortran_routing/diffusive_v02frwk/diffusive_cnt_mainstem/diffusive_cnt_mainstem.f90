...
contains
    ...
    subroutine diffnw(timestep_ar_g, nts_ql_g, nts_ub_g, nts_db_g, ntss_ev_g, nts_qtrib_g, &
        ...
        double precision :: t, tfin, t1, t2, t0, ini_time, tf0 !t0 start time
        ...
        doubleprecision :: cr_val
        integer, dimension(:), allocatable :: dmy_frj
        integer :: jm, usrchj
        doubleprecision :: q_usrch, wdepth

!        open(unit=2, file="./output/fortran diffusive discharge water depth_CNT.txt")
!        open(unit=45, file="./output/courant value_cnt.txt")
        !*---------------------
        !* time step variables
        !*---------------------
        dtini= timestep_ar_g(1) !* dtini_g  !*[sec]
        dtini_given=dtini
        t0= timestep_ar_g(2) !* simulation intial time [hr]
        tfin= timestep_ar_g(3) !* simulation final time [hrs]
        saveInterval= timestep_ar_g(4) !* saving interval [sec]
        ...
        ghost_timenode_n = int(para_ar_g(6)) !* the number of ghost time node, used to get adequate dischage boundary condition in time.
        q_llm = para_ar_g(7) 		     !* 0.02831. lower limit of discharge
        so_llm = para_ar_g(8) 		     !* 0.0001, lower limit of channel bed slope
        wlCalcMethod = int(para_ar_g(9))     !3 !* 1: bisection, 2: simple iterative with contribution from dU/dX, 3: Newton Raphson for water depth
        !*------------------------------
        ...
        do jm=1, nmstem_rch !* mainstem reach only
            ...
            do i=1,ncomp
                ...
                call readXsection(i,(1.0/skLeft(i,j)),(1.0/skMain(i,j)),(1.0/skRight(i,j)),&
                                    leftBank(i,j), rightBank(i,j),timesDepth, j, mxncomp, nlinks, &
...
!                do n=1,nts_ub_g    !* <- ** no head water for mainstem reaches **
        ...
        do kkk = 1,totalTimeSteps-1, repeatInterval
            ...
            do jm=1, nmstem_rch !* mainstem reach only
                ...
                do timestep=1, ntim
                    ...
                    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    !* head water reach
!                    if (frnw_g(j,3)==0) then !<- ** no head-water reach exists among mainstem reach
!                        do n=1,nts_ub_g
!                            varr_ub(n)= ubcd_g(n,j)
!                        end do
...
!                    end if
                    ...
                    if (frnw_g(j,3).gt.0) then  ! reach boundary originates from a junction
                        ...
                        do k=1, frnw_g(j,3)
                            ...
                            if (any(mstem_frj==usrchj)) then
                                ...
                                q_usrch= newQ(frnw_g(usrchj,1), timestep, usrchj)
                            else
                                !* tributary reach through the top node of mainstem segment
                                do n=1,nts_qtrib
                                    varr_qtrib(n)= qtrib(n,usrchj)
                                enddo
                                tc_cnt= ini_time+dtini/60.0*real(timestep-1)
                                q_usrch= intp_y(nts_qtrib, tarr_qtrib, varr_qtrib, tc_cnt)
                            endif
                            ! sum of junction inflows
                            newQ(1,timestep,j) = newQ(1,timestep,j) + q_usrch
                        enddo
                    end if
                    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    !				newY(ncomp,j)= intp_y(nts_db_g, tarr_db, varr_db, tc_cnt)
            ...
            do timestep = 1, repeatInterval+1
                ...
                tc_cnt= ini_time+dtini/60.0*real(timestep-1) !*[min] test
                !print*, timestep, ini_time, ts_ev, tc_cnt
                do j = 1, nlinks
                    ncomp = frnw_g(j,1)
                    if (any(mstem_frj==j)) then
                        !* mainstem upstream reach
                        do i = 1, ncomp
                        ...
                        do k=1, frnw_g(j,3) !* then number of upstream reaches
                            ...
                            if (all(mstem_frj/=usrchj)) then
                                ...
                                elv_ev_g(ts_ev, frnw_g(usrchj,1), usrchj)= newY(1,j)
                                elv_ev_g(ts_ev, 1, usrchj)= wdepth + z(1, usrchj)!* test only
                            endif
                        enddo
                    else
                        !* tributary reach
                        ...
                        enddo
                        tc_cnt= ini_time+dtini/60.0*real(timestep-1) !*[min]
                        q_ev_g(ts_ev, frnw_g(j,1), j)= intp_y(nts_qtrib, tarr_qtrib, varr_qtrib, tc_cnt)
...
!                do j = 1, nlinks
!                    do i = 1, frnw_g(j,1)
!                        write(2,*) tc_cnt, i, j, q_ev_g(ts_ev, i, j), elv_ev_g(ts_ev, i, j)- z(i,j)
!                    enddo
!                enddo
!            enddo
...
! ###########################################################################################
! ###########################################################################################
!     Compute discharge using CNT method, "Algorithms for solving the diffusive wave flood
!       routing equation" by R. Moussa et al.
...
!   Compute water elevation, celerity and diffusivity
    ...
    subroutine readXsection(k,lftBnkMann,rmanning_main,rgtBnkMann,leftBnkX_given,rghtBnkX_given,timesDepth,num_reach,&
                            mxncomp, nlinks, z_ar_g, bo_ar_g, traps_ar_g, tw_ar_g, twcc_ar_g )
        ...
        integer, intent(in) :: mxncomp, nlinks
