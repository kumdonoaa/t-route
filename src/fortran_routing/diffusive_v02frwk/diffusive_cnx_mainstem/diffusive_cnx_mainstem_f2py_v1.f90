...
!                if (i==2) then
!                    if (j==2) then
!                        sumdmy1= sumdmy1 + q_ev_g(ts, i, j)
!                    end if
!                    if (j==7) then
!                        sumdmy2= sumdmy2 + q_ev_g(ts, i, j)
!                    end if
!                endif
!            enddo
!            enddo
...
!            print*, sumdmy1, sumdmy2
    ...
    subroutine C_D_mseg
        ...
        double precision :: c_nom, c_denom, y_norm
        double precision ::  yds, delQ, Qtop, Qbtm, K_sf, sf, dA, dQ
        double precision,dimension(:),allocatable :: Axs_mseg
        ...
        do jm=1, nmstem_rch !* mainstem reach only
            ...
            do i=1, frnw_g(j,1)-1
                ...
                !* compute normal depth at the downstream end sub-node (i.e, the final ghost node)
                Qtop= q_mseg_g(1,i,j)
                Qbtm= q_mseg_g(ncomp_mseg_g(i,j),i,j)
                delQ= (Qtop-Qbtm)/Qbtm
                yds = normdepth(i, j, Qbtm) !* test

                subnode_last = ncomp_mseg_g(i,j) -1
                allocate(Axs_mseg(subnode_last))
...
!                    hxs= max(hxs, mindepth_ns_ar_g(i,j))
                    !* option 3, p5, RM6
                    hxs= diffdepth_sfiter(i, j, Qxs, delQ)
...
!                    c_mseg_g(isubnode,i,j) = (5.0/3.0)*c_nom/c_denom
                    !* option 3: approximated dQ/dA
                    Axs_mseg(isubnode) = Axs
                    if (isubnode<subnode_last) then
                        dQ= q_mseg_g(isubnode+1,i,j) - q_mseg_g(isubnode,i,j)
                        dA= Axs_mseg(isubnode+1)-Axs_mseg(isubnode)
                        if ((abs(dA)<0.001).or.(abs(dQ)<0.001)) then
                            c_mseg_g(isubnode,i,j) = (5.0/3.0)*Qxs/Axs
                        else
                            c_mseg_g(isubnode,i,j) = dQ/dA
                        endif
                    else
                        c_mseg_g(isubnode,i,j) = (5.0/3.0)*Qxs/Axs
                    endif
...
!                    write(42,*) isubnode, i,j, "A:", Axs_mseg(isubnode), c_mseg_g(isubnode,i,j)
!                enddo
                deallocate(Axs_mseg)
            enddo
        enddo
...
!                        q_mseg_g(1,i,j)= intp_y(nts_ub, tarr_ub, varr_ub, tf0) !* tarr_ub in min.
                    ...
                    !* at a junction
                        ...
                        do rch=1, nusrch
                            ...
                            if (any(mstem_frj==usrchj)) then
                                ...
                                do n=1,nts_qtrib
                                    varr_qtrib(n)= qtrib(n,usrchj) !* qlat_g(n,i,j) in unit of m2/sec
                                enddo
                                tf0= tc +  dtini/60.0
    ...
    subroutine elv_calc(ncomp, j)
        ...
        doubleprecision :: yds, Qds, Qus, yus, delQ
...
!            elv_g(i,j)= yi_tp1 + adjz_ar_g(i,j)
            ...
            !* option 2: diffusive depth with sf estimated based on difference b/t upstream and downstream nodes' flows.
            !yds=  elv_g(i+1,j) - adjz_ar_g(i+1,j)
            Qds=  q_g(i+1,j)
            Qus=  q_g(i,j)
            delQ= (Qus - Qds)/Qds
            yus= diffdepth_sfiter(i, j, Qus, delQ)

            elv_g(i,j)= yus + adjz_ar_g(i,j)
        end do
 ...
 !**---------------------------------------------------------------------------------------------
    !*                   Compute diffusive depth by finding Sf iteratively.
    !*      ** Given downstream depth and discharge and upstream discharge, compute upstream depth
    !*      ** i indicate upstream node
    !*      ** p1~2,RM6
    !*
    !*----------------------------------------------------------------------------------------------
    doubleprecision function diffdepth_sfiter(i, j, Qus, delQ)
        implicit none
        integer, intent(in) :: i, j
        double precision, intent(in) ::  Qus, delQ !yds, Qds,
        double precision :: yus, yus_norm
        double precision ::   yrt_inc, yrt_dcr, mtp

        !* option 4:
        yrt_inc= 1.33
        yrt_dcr= 0.67
        mtp= 1.0 !* multiplier to delQ
        yus_norm= normdepth(i, j, Qus)
        yus = (1.0 - mtp*delQ)*yus_norm
        !* ad hoc upper and lower limits of water depth
        if (yus.lt.(yrt_dcr*yus_norm)) then
            yus= yrt_dcr*yus_norm
        elseif (yus.gt.(yrt_inc*yus_norm)) then
            yus= yrt_inc*yus_norm
        endif

        yus= max(yus, mindepth_ns_ar_g(i,j))
        diffdepth_sfiter = yus

    endfunction diffdepth_sfiter
    !*--------------------------------------------
