module diffusive
    ...
    double precision, dimension(:,:,:), allocatable :: depth_mseg_g
    doubleprecision :: dmytc
...
!        double precision, dimension(mxncomp_g, nrch_g), intent(in) :: so_ar_g
        ...
        integer :: jm, nusrch, rch, usrchj
        double precision :: depth, sumdmy1, sumdmy2

!        open(unit=2, file="./output/fortran diffusive discharge water depth_CNX.txt")
!        open(unit=42, file="./output/C_D_mseg diff depth.txt")
!        open(unit=43, file="./output/diff depth by bisection.txt")
!        open(unit=44, file="./output/elv C D test.txt")
!        open(unit=45, file="./output/courant.txt")
        !*---------------------
        !* time step variables
        !*---------------------
        ...
        allocate(qtrib(nts_qtrib, nrch_g))
        allocate(dmy_frj(nrch_g))
        allocate( depth_mseg_g(mxncomp_mseg, mxncomp_g, nrch_g) )
...
!        enddo
        ...
        do while (tc .lt. tfin*60.0)
            do j=1,nrch_g
                if (all(mstem_frj/=j)) then
                    ...
                    enddo
                    q_ev_g(ts, frnw_g(j,1), j)= intp_y(nts_qtrib, tarr_qtrib, varr_qtrib, tc)
                    q_ev_g(ts, 1, j)= q_ev_g(ts, frnw_g(j,1), j)
                endif
            enddo
        ...
        enddo
        !**--------------------------------------------------------------------------------------------------*
        !*       Let's start actual diffusive routing over entire time step from here.
        !*
        !**--------------------------------------------------------------------------------------------------*
        ts=1
        !do j=1, nrch_g
        do jm=1, nmstem_rch !* mainstem reach only
            ...
            do rch=1, nusrch
                ...
                if (all(mstem_frj/=usrchj)) then
                    ...
                    depth= elv_g(1,j)-adjz_ar_g(1,j)
                    elv_ev_g(ts, frnw_g(usrchj,1), usrchj)= z_ar_g(frnw_g(usrchj,1), usrchj)+ depth
                    elv_ev_g(ts, 1, usrchj)= z_ar_g(1, usrchj)+ depth !* test only
                endif
            enddo
        enddo
        ...
        do while (tc .lt. tfin*60.0)
            ...
            do jm=1, nmstem_rch !* mainstem reach only
                ...
                do i=1, frnw_g(j,1)-1
                    ...
                    !write(45,*) dmytc, i, j, c_g(i,j),  courant_comp(i,j)
                enddo
            end do
            !**--------------------------------------------------------------------------------------------
...
!                endif
            ...
            do jm=nmstem_rch, 1, -1 !* mainstem reach only
                ...
                if (frnw_g(j,2)<0.0) then
                !* downstream boundary node at TW
                !* option 1. measured water depth data at TW node
!                    do n=1,nts_db_g
...
!                    elv_g(ncomp,j)= intp_y(nts_db_g, tarr_db, varr_db, tf0)
            ...
            do jm=1, nmstem_rch !* mainstem reach only
                ...
                ncomp= frnw_g(j,1)
                do i=1, ncomp
                    q_ev_g(ts, i, j)=q_g(i,j) !** tributary flow from MC is already stored to q_ev_g before this time loop.
                    elv_ev_g(ts, i, j)= z_ar_g(i,j)+ elv_g(i,j)-adjz_ar_g(i,j)
!                        !if ((tc.ge.21000).and.(tc.le.28000)) then
!                        !if (j==689) then
!                        !endif
!                        !endif
...
!            do j=1, nrch_g
!            do i=1, frnw_g(j,1)
!                write(2,"(f10.1, 2I10, 2F20.4)") dtini*real(ts-1)/60.0, i, j, q_ev_g(ts, i, j), elv_ev_g(ts, i, j)-z_ar_g(i,j)
!                !if (i==2) then
!                    if (j==2) then
!                        sumdmy1= sumdmy1 + q_ev_g(ts, 2, j)
!                    end if
!                    if (j==3) then
!                        sumdmy2= sumdmy2 + q_ev_g(ts, 1, j)
!                    end if
!                !endif
!            enddo
!            enddo
...
!                    aved = d_g(i,j)
                    !* option 2:
                    avec= c_mseg_g(isubnode, i, j)
                    aved= d_mseg_g(isubnode, i, j)
    ...
    subroutine Q_mseg_resoltech(tc, ubcd_g)
        ...
        doubleprecision, intent(in) :: tc
        double precision, dimension(nts_ub, nrch), intent(in) :: ubcd_g
        integer :: i, j, jm, nusrch, rch, usrchj
        double precision :: dxst
        double precision :: qjt
        double precision, dimension(:), allocatable :: x, y
        integer :: ncomp_ms
        integer :: isubnode, n, subnode_last, subnode
        double precision :: tf0,totalx, x1, x2, q1_mseg, q2_mseg, q_usrch
...
!                        q_mseg_g(1,i,j)= intp_y(nts_ub, tarr_ub, varr_ub, tf0) !* tarr_ub in min.
                    ...
                    !* at a junction
                        ...
                        do rch=1, nusrch
                            ...
                            if (any(mstem_frj==usrchj)) then
                                ...
                                q_usrch= q_mseg_btnode(frnw_g(usrchj,1)-1, usrchj)
                            else
                ...
                !* Q at the top sub-node of any segment after the first one is equal to Q at the last sub-node of its just upstream segment.
                if (i.ge.2) then
               ...
               !* x and y computation in backward-sweep
                ...
                !* estimate Q at the bottom node of segment i
                totalx= dx_ar(i,j)
    ...
    subroutine elv_calc(ncomp, j)
        ...
        doubleprecision :: yds, Qds, Qus, yus, delQ, ybtnode
        doubleprecision, dimension(:), allocatable :: depth_mseg_1d, q_mseg_1d, sf_mseg_1d
        integer :: ncomp_ms, ncomp_ms2
        double precision :: chbed_slp, Qxs, hxs, Axs, Bxs,  sumc, avec, sumd, aved
        double precision :: Bxs1, Bxs2, Qxs1, Qxs2, c_nom, c_denom, emann
        integer :: isubnode, subnode_last

        !* compute water elevation at the next time step
        do i=ncomp-1,1,-1
            !* option 4:  diffusive depth using Bisection method
            ybtnode= elv_g(i+1,j) - adjz_ar_g(i+1,j)
            ncomp_ms= ncomp_mseg_g(i,j)
            allocate(depth_mseg_1d(ncomp_ms), q_mseg_1d(ncomp_ms),  sf_mseg_1d(ncomp_ms))
            depth_mseg_1d=0.0
            q_mseg_1d=0.0
            sf_mseg_1d=0.0

            ybtnode= elv_g(i+1,j) - adjz_ar_g(i+1,j)
            ybtnode= max(ybtnode, mindepth_ns_ar_g(i,j))

            call diffdepth_BS(i, j, ncomp_ms, depth_mseg_1d, q_mseg_1d, sf_mseg_1d, ybtnode, ncomp_ms2)
            elv_g(i,j)= depth_mseg_1d(1) + adjz_ar_g(i,j)
            ...
            subnode_last= ncomp_ms2+1
            do isubnode= subnode_last, 1, -1
                Qxs= q_mseg_1d(isubnode)
                hxs= depth_mseg_1d(isubnode)
...
!                if (isubnode.eq.subnode_last) then
!                    Qxs2= q_mseg_1d(isubnode)
!                else
!                    Qxs2= q_mseg_1d(isubnode+1)
!                endif
!                Qxs1= q_mseg_1d(isubnode)
!                emann= emann_tmrf(hxs) !* equivalent manning's N
!                Bxs= topwidth(hxs)
!                delQ= (Qxs1 - Qxs2)/Qxs2
!                sf= (1.0+ delQ)*chbed_slp
!                sf = max(sf, chbed_slp)
!                !c_nom = (chbed_slp**0.3)*(Qxs**0.4)
!                c_nom = (sf**0.3)*(Qxs**0.4)
...
!                c_mseg_g(isubnode,i,j) = (5.0/3.0)*c_nom/c_denom
                ...
                !**-----------------------------------------------------
                if (isubnode<subnode_last) then
                    !* option 1:
!                    Bxs= topwidth(hxs)
!                    sf= sf_mseg_1d(isubnode)
!                    d_mseg_g(isubnode,i,j) = Qxs/(2.0*Bxs*sf)
                    !* option 2:
                    hxs= depth_mseg_1d(isubnode+1)
                    Bxs2= topwidth(hxs)
                    hxs= depth_mseg_1d(isubnode)
                    Bxs1= topwidth(hxs)
                    Bxs= 0.5*(Bxs1+Bxs2)
                    ...
                    sf= (1.0+ delQ)*chbed_slp
                    sf = max(sf, chbed_slp)
                    d_mseg_g(isubnode,i,j) = (Qxs1+Qxs2)/(2.0*Bxs*sf)
                    d_mseg_g(isubnode,i,j) = min(d_mseg_g(isubnode,i,j), 1000.0)
                endif
            enddo !* do isubnode= subnode_last, 1, -1
            !* option 1
            ...
            sumd= sum(d_mseg_g(1:(subnode_last-1), i, j))
            aved= sumd/real(subnode_last-1)
            d_g(i,j) = aved
!            do i2=1, subnode_last
!                write(44,*) dmytc, i2, i, j,  q_mseg_1d(i2), depth_mseg_1d(i2),&
!                        sf_mseg_1d(i2), c_mseg_g(i2,i,j), d_mseg_g(i2,i,j),&
!                        c_g(i,j), d_g(i,j), courant_comp(i,j)
!            enddo
            do isubnode= subnode_last+1, ncomp_ms
                c_mseg_g(isubnode,i,j)= c_mseg_g(subnode_last, i, j)
            enddo
            do isubnode= subnode_last, ncomp_ms
                d_mseg_g(isubnode,i,j)= d_mseg_g(subnode_last-1, i, j)
            enddo

            deallocate(depth_mseg_1d, q_mseg_1d,  sf_mseg_1d)
    ...
    !**---------------------------------------------------------------------------------------------
    !*           Compute diffusive depth by estimating average Sf using Bisection
    !*      ** p10, RM6
    !*      ** Bisection by NumericalRecipesF90_vol2, p1185
    !*----------------------------------------------------------------------------------------------
    subroutine diffdepth_BS(i, j, ncomp_ms, depth_mseg_1d, q_mseg_1d, sf_mseg_1d, ybtnode, ncomp_ms2)
        implicit none
        integer, intent(in) :: i, j, ncomp_ms
        double precision, intent(in) :: ybtnode
        integer, intent(out) :: ncomp_ms2
        double precision, dimension(ncomp_ms), intent(out) :: depth_mseg_1d, q_mseg_1d, sf_mseg_1d
        double precision :: rtbis
        !*using bisection, find the root of a function fy known to lie between x1 and x2. the
        ...
        integer :: iter, maxit
        double precision :: x1, x2, xacc, dx, f, fmid, xmid
        integer :: isubnode
        double precision :: yus_mseg, yds_mseg, qus_mseg, qds_mseg, kus_mseg, kds_mseg, sfds_mseg
        double precision :: dxseg, dxmseg, tx_mseg, dxmseg2, dx2
        double precision :: chbed_slp, delQ
        double precision :: C1, C2, yll, yul, fyul, fyll

        maxit=40
        dxseg= dx_ar(i,j)
        dxmseg = dx_mseg_g(i,j)

        !* find the value of isubnode that mark beginning of a subnode before bottom node of i segment.
        tx_mseg= dxmseg*real(isubnode-1)
        do isubnode= ncomp_ms,1,-1
            tx_mseg= dxmseg*real(isubnode-1)
            if (tx_mseg.le.dxseg) then
                dxmseg2= dxseg-tx_mseg
                ncomp_ms2= isubnode
                exit
            endif
        enddo
        !* store values at the bottom node of i segment for C and D calculation in subroutine elev
        depth_mseg_1d(ncomp_ms2+1)= ybtnode
        q_mseg_1d(ncomp_ms2+1)= q_g(i+1,j)

        chbed_slp = max(so_llm, adjso_ar_g(i,j))
        do isubnode= ncomp_ms2, 1, -1
            iter=0 !* to count iteration in Bisection method.
            if (isubnode==ncomp_ms2) then
                if (dxmseg2.lt.(0.01*dxmseg)) then
                    !* when distance between upstream and downstream sub-nodes is too close, then take the equal depth value.
                    dx2= 0.0 !*yus_mseg= yds_mseg
                else
                    dx2= dxmseg2
                endif
                qds_mseg= q_g(i+1,j)
                ...
                dx2= dxmseg
                qds_mseg= q_mseg_g(isubnode+1,i,j)
                yds_mseg= yus_mseg
            endif
            qus_mseg= q_mseg_g(isubnode,i,j)
            kds_mseg= convey(i, j, yds_mseg)
            sfds_mseg = qds_mseg*abs(qds_mseg)/kds_mseg**2.0
            if (dx2.gt.0.0) then
                !* apply newton rapson to estimate water depth at upstream sub-node.
                !* compute values of variables at downstream sub-node with known Q and y
                !* K and Sf at downstream sub-node
                C1= 0.5*qus_mseg*abs(qus_mseg)*dx2
                C2= -yds_mseg + chbed_slp*dx2 - 0.5*sfds_mseg*dx2

                !* find lower and upper limit of yus_mseg that gives f_ll*f_ul <0.0
                yll= 0.5*yds_mseg
                yul= 1.5*yds_mseg
                fyll= fy(i,j, C1, C2, yll)
                fyul= fy(i,j, C1, C2, yul)
                if (fyll*fyul>=0.0) then
                    !* second and last try of guessing the limits
                    yll= mindepth_ns_ar_g(i,j)
                    yul= 1.5*yul
                    fyll= fy(i,j, C1, C2, yll)
                    fyul= fy(i,j, C1, C2, yul)
                end if

                if (fyll*fyul>=0.0) then !* when still limits are not found yet,
                    !* best ad-hoc estimate
                    delQ= (qus_mseg - qds_mseg)/qds_mseg
                    delQ= sign(min(abs(delQ), 0.33), delQ)
                    yus_mseg= (1.0 - delQ)*yds_mseg
                else
                    x1= yll
                    x2= yul
                    !**----------------------------------
                    !*      Bisection core kernel
                    !**----------------------------------
                    xacc = 0.01*yds_mseg
                    f= fy(i,j, C1, C2, x1)
                    fmid= fy(i,j, C1, C2, x2)
                    if (f < 0.0) then  !* orient the search so that f>0 lies at x+dx (i.e., f(rtbis) always <0)
                        rtbis=x1
                        dx=x2-x1
                    else
                        rtbis=x2
                    ...
                    do iter=1,maxit !* bisection loop.
                        dx=dx*0.5
                        xmid=rtbis+dx
                        fmid=fy(i,j, C1, C2, xmid)
                    ...
                    enddo
                endif !* if (fyll*fyul>=0.0) then
            else !*if (dx2.gt.0.0) then
                yus_mseg= yds_mseg
            endif

            yus_mseg= max(yus_mseg, mindepth_ns_ar_g(i,j))
            depth_mseg_1d(isubnode)= yus_mseg
            q_mseg_1d(isubnode)= qus_mseg

!            kus_mseg= convey(i, j, yus_mseg)
!            sf_mseg_1d(isubnode)= 0.5*(qus_mseg*abs(qus_mseg)/kus_mseg**2.0 + sfds_mseg)
!            if (sf_mseg_1d(isubnode)>=1.0) then
!                delQ= (qus_mseg - qds_mseg)/qds_mseg
!                sf_mseg_1d(isubnode)= (1.0+ delQ)*chbed_slp
!            endif
!            sf_mseg_1d(isubnode)= max(sf_mseg_1d(isubnode), so_llm)
!            write(43,*) dmytc, isubnode, i, j, iter, dx2, chbed_slp, qus_mseg, qds_mseg,&
!                        yus_mseg, yds_mseg
         enddo

    endsubroutine diffdepth_BS
    !**--------------------------------------------------------------------
    !*            f(y at upstream node)=0 for diffusive momentum eq. p10,RM6
    !**--------------------------------------------------------------------
    doubleprecision function fy(i,j, C1, C2, y)
        implicit none
        integer, intent(in) :: i, j
        doubleprecision, intent(in) :: C1, C2, y
        doubleprecision :: ky
